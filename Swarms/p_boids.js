...
class BoidSystem extends ParticleSystem {
  ...
  static desc = "Wolf Pack"; // Ignore the Glitch parse error

  constructor() {
    // Make what particle, and how many?
    // Try different numbers of particles
    super(BoidParticle, 20);
...
class BoidParticle extends Particle {
  constructor(ps, index) {
    ...
    this.v.setToPolar(50, this.angle);
  ...
  calculateForces(p, dt) {
    ...
    // Cohesion
    // Move toward center
    this.cohesionForce = this.pos.getForceTowardsPoint(this.ps.flockCenter, 1, {falloff: 2})
    ...
    this.propulsionForce.setToPolar(120*flyingStrength, 
                                    this.angle + turn)
    
    // Chase the mouse?
    let mouse = new Vector2D(p.mouseX, p.mouseY)
    this.attractionForce = this.pos.getForceTowardsPoint(mouse, 1, {falloff: 2})
    
    // let black_hole = new Vector2D(p.width + 125, p.height + 125)
    // this.attractionForce = this.pos.getForceTowardsPoint(black_hole, 1, {falloff: 2})
    ...
    this.cohesionForce.mult(1)
    this.alignmentForce.mult(0.5)
