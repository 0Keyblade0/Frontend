...
class WindParticle extends Particle {
  ...
  calculateForces(p, dt) {
    ...
    // they clump up
    windDir = Math.sin(20*p.noise(windX, windY))
    ...
    this.windForce.setToPolar(10, windDir);
    this.f.sub(this.windForce);
    
    let mouse = new Vector2D(p.mouseX, p.mouseY)
    this.attractionForce = this.pos.getForceTowardsPoint(mouse, 1, {falloff: 0.9})
    this.f.sub(this.attractionForce )
  }
