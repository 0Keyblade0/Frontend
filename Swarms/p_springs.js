...
class Edge {
  ...
  applyForces() {
    ...
    if (this.length > 0) {
      ...
      this.pt0.springForce.addMultiple(this.edgeVector, springAmt/5);
      this.pt1.springForce.addMultiple(this.edgeVector, -springAmt/10);
    }
  }
...
class SpringSystem extends ParticleSystem {
  ...
  static desc = "Imaginary Springs - steady state is impossible"; // Ignore the Glitch parse error
  ...
  constructor() {
    ...
    for (var i = 0; i < 2; i++) {
      this.particles.forEach((pt, index) => {
        ...
        // Create a new edge
        let e = new Edge(pt, pt1, { length: 100 + Math.random()*100, strength: 2 + Math.random()*2 });
