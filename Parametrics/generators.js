const GENERATORS = {
  ...
  planet: {
    ...
    landmarks: {
      jupiter: [0.95, 0.11, 0.2, 0.0, 0.54, 0.78, 0.73],
      saturn: [0.26, 0.96, 0.49, 0.28, 0.51, 0.87, 0.54],
      mercury: [0.0, 0.74, 0.46, 0.8, 0.0, 0.09, 0.27],
      rainbowPlanet: [0.0, 1.0, 0.2, 0.46, 1.0, 0.78, 1.0],
      earth: [0.16, 0.7, 0.2, 0.36, 0.1, 0.78, 0.0],
    },
    ...
    draw(p, t, dna) {
      ...
      function drawImage() {
        ...
        for (var i = 0; i < 40; i++) {
          ...
          let pastel = 70 + 80 * p.noise(i * 10) - 0.6 * sat;
          hue2 % -360;
  ...
  Balls: {
    description: "bouncing balls with eyes (practice)",
    sliders: ["size", "aspectRatio", "angle", "hue", "brightness", "jumpDistance"],
    landmarks: {
      restingGolfball: [0.00,0.50,0.80,0.16,0.50,0.00],
      basketball: [0.70,0.00,0.50,0.10,0.36,0.52],
      tableTennisBall: [0.03,0.00,0.03,0.18,0.44,1.00],
      bowlingball: [1.00,0.38,1.00,0.36,0.00,0.00],
      wiffleball: [0.00,1.00,1.00,1.00,1.00,1.00]
    },
    setup(p) {},

    drawBackground(p) {
      p.background(100, 100, 100, 0.1);
    },
  ...
  MineralDeposit: {
    ...
    landmarks: {
      goldNugget: [0.00, 0.00, 0.00, 0.50, 0.14, 0.50, 0.50, 0.40, 0.30],
      roseQuartz: [0.80, 0.54, 0.86, 0.20, 0.02, 0.62, 1.00, 1.00, 0.30],
      amethyst: [0.32, 0.80, 0.06, 1.00, 0.76, 0.54, 0.50, 1.00, 1.00],
      timeStone: [0.30, 0.04, 0.38, 0.92, 0.30, 0.50, 0.48, 1.00, 0.20],
      diamond: [0.26, 0.30, 0.66, 0.66, 0.50, 0.56, 0.58, 0.34, 0.60]
    },
    setup(p) {},

    drawBackground(p) {
      p.background(19, 44, 36, 0.1);

      for (var i = 0; i < 50; i++) {
        p.fill(0, 0, 0);
        p.circle(Math.random() * p.width, Math.random() * p.height, 2);
      }
    },
    ...
    draw(p, t, dna, index) {
      ...
      let mineral_size = dna[0] * 50 + 10;
      let width = dna[1] * 30
      let height = dna[2] * 30
      let angle = dna[3] - 0.5;
      let hue = dna[4];
      let hueOffset = dna[5] - 0.5;
      let brightness = dna[6] * 100
      let concealment = dna[7]
      let cut = dna[8] * 10
      
      p.translate(0, -100 + 100 * p.noise(0.2 + t + index));
      p.rotate(angle + p.noise(0.8 * t + index) - 0.5);

      // draw 3 shapes on top of each other
      for (var i = 0; i < cut; i++) {
        let size = mineral_size * (1 - i * 0.2);
        p.fill(((hue + 0.2 * i * hueOffset) % 1) * 360, 100, brightness - i * 10 , concealment);
        p.beginShape();
        p.vertex(0, -50 + size + height);
        p.vertex(size + width, -50);
        p.vertex(0, -50 - size - height);
        p.vertex(-size - width, -50);
